<template>
  <div>
    <div class="header-img">
      <span class="header-text">Subscription</span>
    </div>
    <div class="container">
      <div class="row breadcrumb" style="background: transparent;">
        ...
        <ul>
          <li class="bullet">Subscription</li>
        </ul>
      </div>
    </div>
    <div class="container responsive_container mb-5">
      <div class="row mt-3">
        <div class="col-12">
          <h2 class="text-theme" style="font-weight:700">
            Subscription
          </h2>
        </div>
      </div>
    ...
    methods: {
      postTrainerInquiry: function() {
        if (localStorage.getItem('token')) {
          ...
          payload.append('plan', $('#plans').val())
          this.$store.dispatch('postSub', payload).then(res => {
            alert('Dear ' + this.firstname + ', we will attend to you shortly!')
            this.$router.push('/')
          })
        } else {
          alert('Please Login')
      ...
      getPlans: function() {
        this.$store.dispatch('getPlans').then(res => {
          ...
          res.data.map(item => {
            brandObj[item.id] = { name: item.title }
          })
